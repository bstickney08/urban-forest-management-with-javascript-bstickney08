...
const parkIndex = parks.findIndex(park => park.name === "Riverside Park");

if (parkIndex !== -1) {
  parks[parkIndex].name = "Riverside Greenspace";
}
console.log(parks);
...
const centralPark = parks.find(park => park.name === "Central Park");
const centralParkTrees = centralPark ? centralPark.trees.map(tree => tree.species) : [];
...
const tallestTree = parks.flatMap(park => park.trees).reduce((tallest, tree) => (tree.height > tallest.height ? tree : tallest), { height: 0 });
...
if (parks.length > 0) {
  ...
  console.log("Name:", firstPark.name);
  console.log("Facilities:", firstPark.facilities);
} else {
  console.log("The parks array is empty.");
}

// Task 10: Using console.log, display the species of the third item in the parks array.
// Your code here

if (parks.length > 2 && parks[2].trees && parks[2].trees.length > 0) {
  const thirdParkThirdTreeSpecies = parks[2].trees[2].species;
  console.log("Species of the third tree in the third park:", thirdParkThirdTreeSpecies);
} else {
  console.log("Unable to retrieve the species.");
}
